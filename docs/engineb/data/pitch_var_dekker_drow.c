/* eb_pitch.c — VARIANT B2 "dekker_drow" (dekker eval + double row) (PROBE BUILD, decision measurement only).
 *
...
/* DIAGNOSTIC HYBRID: clamp and row are the PORT'S OWN double forms, verbatim
 * from engine_b/eb_pitch.c. Only the polynomial evaluation is double-float.
 * This attributes the 2/30 dekker failures to either row/clamp (if this
 * passes) or df eval precision (if this still fails). */
