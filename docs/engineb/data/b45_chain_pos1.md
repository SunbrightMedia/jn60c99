...
## POS4 RE-FLASH (VERSION 20260914213344, log 20260914_143809): MUTES AGAIN --
## DETERMINISTIC, NOT THE PARAM CODE

Same four bins, one clean run. POS1/POS2/POS3 MATCH (0 bad), play the storm,
end SILENT rg=dg=0, STUCK=0. POS4 mutes again. The detailed line, IDENTICAL in
BOTH runs (211631 + 213344):
  POS4 CRC MISMATCH patch 0: chip rc=ebdfa32b mc=9a07774b |
                             host rc=c6c5d9aa mc=9a07774b
Reads:
 - DETERMINISTIC: chip rc=ebdfa32b both runs -> not random PSRAM read noise.
 - MASTER coefs MATCH (mc=9a07774b == host); only the VOICE coefs (rc) differ.
 - devcrc_rc[0]=c6c5d9aa is the checked-in base-0 answer key; 204047 (before
   this change) POS4 MATCHED it.

NOT THE PARAM CODE, by construction:
 - The answer key is generated by tools/engineb/devboot/devcrc.c from the HOST
   eb_devseq.c -> eb_coefs.c. This change touches NONE of them; eb_devparam.h
   is included only by the firmware main + the devparam gate, never by the
   coefficient path. So the coefficient VALUES are byte-unchanged by this
   change, and the answer key is unchanged.
 - The new globals (pm_leaf_*, ~120 B) are INTERNAL DRAM, so they do not shift
   EBDEV_S (EXT_RAM_BSS_ATTR, PSRAM). No PSRAM layout move.
 - POS1/2/3 run the same coefficient logic and MATCH their keys.

CONCLUSION: a deterministic, POS4-only, voice-coef-only boot mismatch that is
outside the code this change modifies. Most likely a marginal/stuck voice-cell
region on the POS4 DevKit's PSRAM (204047 -> now the same board degraded, or a
bad flash that reproduces). DECISIVE test the user can run: flash the PARENT
bins (commit 23111f2, pre-change) to the POS4 slot -- if POS4 STILL mismatches,
the board is at fault, independent of this change; if it matches, the board is
