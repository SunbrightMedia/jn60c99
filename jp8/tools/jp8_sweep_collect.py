...
usage: jp8_sweep_collect.py [logdir] [out.md] [drive]   (drive "drive2" = the constructed-HOST oracle, S3_STATUS D7;
omitted = the pre-drive2 text, byte-identical to the old SWEEP_44100.md header)"""
...
drive=sys.argv[3] if len(sys.argv)>3 else None
rows={}; missing=[]
for p in range(64):
...
with open(outp,"w") as o:
    if drive=="drive2":
        rel=os.path.relpath(os.path.abspath(logdir),os.path.join(HERE,"..",".."))
        o.write("# %s -- 64-patch listen sweep of the JP8 oracle at 44100 on DRIVE2 (PORT_PIPELINE step 4 reach)\n\n"%os.path.basename(outp))
        o.write("Produced by `jp8/tools/jp8_sweep.py <patch> <logdir>` on DRIVE2 (S3_STATUS D7: HOST built by the plugin's factory 0x444FE0,\n")
        o.write("BUILD -> FTZ -> SETSR -> host_init -> recall through HOSTPARAM 0x4465B0 per pool; NO snap, NO latch clear; ramp census per\n")
        o.write("stage) and collected by `jp8_sweep_collect.py <logdir> <out> drive2`; every row has its log in `%s/pNN.log`. Labels: PROVEN (executed on\n"%rel)
    else:
...
print("%s: %d/64 PASS, fails %s, missing %s"%(os.path.basename(outp),npass,fails,missing))
