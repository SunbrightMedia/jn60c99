...
# lift with ONE addss turned into subss (jp8_lift_seq.TOOTH per layer) must FAIL.
set -e
HERE="$(cd "$(dirname "$0")" && pwd)"; REPO="$(cd "$HERE/../.." && pwd)"; OUT="$REPO/build/jp8_lift"; mkdir -p "$OUT"
DYN="$OUT/dynreach.json"; DYNB="$OUT/dynreach_boot.json"; LAYER="${JP8_LIFT_LAYER:-render}"; REF="$OUT/ref_$LAYER"; export JP8_LIFT_LAYER="$LAYER"
