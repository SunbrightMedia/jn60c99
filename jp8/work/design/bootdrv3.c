...
static void snap_all(void) {   /* PROBE ONLY: jp8_emu.snap_ramps + clear_latch, harness law, not plugin code */
    int u; for (u = 0; u < 9; u++) { uint64_t s = st[u], arr = rq(s + 0x58), b0 = rq(s + 0x70), e0 = rq(s + 0x78), a; long n = (long)(e0 - b0) / 4, i;
        for (i = 0; i < n; i++) { int32_t id; memcpy(&id, (void *)(uintptr_t)(b0 + 4 * i), 4); a = arr + 40 * (uint64_t)id;
            { uint64_t t = rq(a); if (t) memcpy((void *)(uintptr_t)t, (void *)(uintptr_t)(a + 0x14), 4); }
            memset((void *)(uintptr_t)(a + 0xC), 0, 4); *(uint8_t *)(uintptr_t)(a + 0x1C) = 0; }
        wq(s + 0x78, b0); memset((void *)(uintptr_t)(s + 0xA9C0B8), 0, 4); } }
static void rcensus(const char *tag) {  /* PROBE: live ramp census of unit 0 and 8 */
    int u; for (u = 0; u < 9; u += 8) { uint64_t s = st[u], arr = rq(s + 0x58), b0 = rq(s + 0x70), e0 = rq(s + 0x78); long n = (long)(e0 - b0) / 4, i, nn = 0, ninf = 0, nacc = 0, ntn = 0;
        for (i = 0; i < n; i++) { int32_t id; float sg, ac, tv; uint64_t a, t; memcpy(&id, (void *)(uintptr_t)(b0 + 4 * i), 4); a = arr + 40 * (uint64_t)id;
            memcpy(&sg, (void *)(uintptr_t)(a + 8), 4); memcpy(&ac, (void *)(uintptr_t)(a + 0xC), 4); t = rq(a);
            if (sg != sg) nn++; else if (sg - sg != 0) ninf++; if (ac != ac) nacc++; if (t) { memcpy(&tv, (void *)(uintptr_t)t, 4); if (tv != tv) ntn++; } }
        printf("  [%s] unit %d: %ld live ramps, step NaN %ld, step inf %ld, accum NaN %ld, target NaN %ld\n", tag, u, n, nn, ninf, nacc, ntn); } }
static void rlist(void) { uint64_t s = st[0], arr = rq(s + 0x58), b0 = rq(s + 0x70), e0 = rq(s + 0x78); long n = (long)(e0 - b0) / 4, i;
    for (i = 0; i < n; i++) { int32_t id; float sg, ac, bs, lm; int32_t per, cnt; uint64_t a, t; memcpy(&id, (void *)(uintptr_t)(b0 + 4 * i), 4); a = arr + 40 * (uint64_t)id;
        memcpy(&sg, (void *)(uintptr_t)(a + 8), 4); memcpy(&ac, (void *)(uintptr_t)(a + 0xC), 4); memcpy(&bs, (void *)(uintptr_t)(a + 0x10), 4); memcpy(&lm, (void *)(uintptr_t)(a + 0x14), 4);
        memcpy(&per, (void *)(uintptr_t)(a + 0x20), 4); memcpy(&cnt, (void *)(uintptr_t)(a + 0x24), 4); t = rq(a);
        if (sg != sg || i < 3) printf("    id %4d tgt st+0x%06llx step %g acc %g base %g lim %g per %d cnt %d\n", id, (unsigned long long)(t ? t - s : 0), sg, ac, bs, lm, per, cnt); } }
static uint64_t fnv(uint64_t a, size_t n) { uint64_t h = 1469598103934665603ULL; const unsigned char *p = (const unsigned char *)(uintptr_t)a; size_t k; for (k = 0; k < n; k++) h = (h ^ p[k]) * 1099511628211ULL; return h; }
int main(int argc, char **argv) {
    ...
    mx = bmx; t0 = now(); call(BUILD, host, 0, 0, 0); printf("BUILD @%04x: %.3f s, heap used %.1f MB, heap fnv %016llx\n", bmx, now() - t0, (jp8_heap_get() - HEAP) / 1e6, (unsigned long long)fnv(HEAP, jp8_heap_get() - HEAP));
    mx = 0x9FC0;
    ...
    printf("SETSR: %.3f s\n", now() - t0); rcensus("after SETSR");
    t0 = now(); for (i = 0; i < nh; i++) call(HOSTPARAM, host, hid[i], hval[i], 0); printf("host_init (%d writes): %.4f s\n", nh, now() - t0);
    rcensus("after host_init");
    int smode = argc > 7 ? atoi(argv[7]) : 0;
    if (smode == 1) { snap_all(); printf("SNAP before recall (T1)\n"); }
    ...
    rcensus("after recall");
    if (getenv("RLIST")) rlist();
    if (smode == 2) { snap_all(); printf("SNAP after recall (step-4 recipe)\n"); }
    if (smode) goto longrun;
    cap = 2328 * 72; words = malloc(cap); nw = 0;
    if (argc > 7 && atoi(argv[7])) goto longrun;
    ...
    free(words); words = 0;
longrun:
    { int k; for (k = 0; k < 20; k++) sample(); rcensus("after 20 samples"); if (getenv("RLIST")) rlist(); }
    /* continue: long idle, then a note -- windowed peaks */
