#!/usr/bin/env python3
"""burst_gate.py -- build and run O2's note-sequence gate.
...
OUT  = os.path.join(REPO, "build", "burst_gate")
os.makedirs(os.path.dirname(OUT), exist_ok=True)

cmd = ["cc", "-std=c99", "-O1", "-Wall", "-Wextra", "-Werror",
       "-I", os.path.join(REPO, "engine_b", "dev"),
       os.path.join(HERE, "devboot", "burst_gate.c"), "-o", OUT]
r = subprocess.run(cmd)
if r.returncode:
    print("burst_gate: COMPILE FAILED"); sys.exit(2)
