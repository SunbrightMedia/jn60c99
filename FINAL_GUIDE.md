...
| **O3** | C9 | per-parameter incremental refresh (derived field map) | **BUILT, GATED AND COMPILED FOR THE TARGET; UNFLASHED.** A knob writes its two record bytes, recalls WARM (no reseed, no install) and rebuilds ONLY the sub-builders its class names. MEASURED (data/b13_param_map.md): of 59 parameters the median moves **32 of 12,276 coefficient bytes (0.26%)**, worst 471 (3.84%) — the full rebuild a program change earns is ~380x a knob's work. LEGALITY settled, answering eb_devseq.h's standing warning: **2,036 of 2,040 parameters land byte for byte where a cold recall would**; the four that diverge are transition LATCHES that settle, where WARM is correct for a live edit. `eb_param_class.h` is GENERATED and re-derived every gate run, so a stale table fails before it ships a stale coefficient. Third state machine (`eb_paramstep.h`) built to the publish contract FROM ITS FIRST LINE rather than corrected into it (playbook 62). Budget-gated, where the patch burst may not be: its worst step (warm recall ~0.24 M) FITS the ~0.46 M slack; the burst's 591,526 never can. **57 teeth across the suite, all caught** (`sh tools/engineb/o3_gates.sh`), including the b11 starvation regression replayed as an interlock tooth. Coalescing proven: 4,000 edits cost 575 builds. **REMAINING: silicon** — `PARAM: unknown=` and `pubretry=` must read 0, and the refresh rate measured by b12's `B4dur` method. ⚠ A knob can wait **48 blocks (~278 ms)** behind a sustained note storm; that priority is a POLICY the user may overturn. ⚠ NOT DONE: N (the refresh period) is the user's pick off the measured zipper curve (F2). |
| **O4** | B3/B4 | worst-case headroom CLOSED — the only holder of the deadline deficit | **OPEN, and its FIRST QUESTION IS NOT A CYCLE HUNT (data/b14_o4_open.md).** b12's durations RECONCILE — implied drift 149.5 s vs the board's own 150.1 s, agreement 0.4 %, two clocks and two code paths — so the target is real. Restated correctly: the run mean is **306.6 µs/block over period = 287 cyc/sample**, actual ~5,729 against the 5,442 budget. (The 197 µs in b12 is the QUIET class, i.e. the floor, not the average — both true, different questions.) b6 measured the RENDER at 5,112-5,389, so **340-620 cyc/sample live outside the render measurement** and must be attributed before any lever is chosen. ⚠ **A HYPOTHESIS COULD DISSOLVE THE WHOLE STEP**: the loop blocks in `i2s_channel_write(portMAX_DELAY)`, so when the engine is AHEAD the block interval is the DAC's period, not the CPU's cost — 6,001 µs would then mean an effective 42,660 Hz clock, no cycle deficit at all, and it would also explain the `ovr_late` anomaly open since b4 §5. **The discriminator is ALREADY PRINTED** by the `I2S: blocked …/… zero-block …` line: zero-block high = engine behind (cycle hunt); blocked large = engine parked on the DAC (clock fix). READ IT off the next log before writing any O4 code. Then (a) prologue (`S3L_TIME_PROLOGUE`, still 0), (b) the delay arm's +1.45x, (c) the 4-vs-18 classification mismatch, (d) only then the lever. Evidence base still b6_split_sweep.md; keep split 7. |
