...
| **O2** | C10 | chunked patch change — THE fix for B4's counted misses | **BUILT, GATED, ~20x BETTER, NOT DONE** (b7): misses fell from 1-4 PER program change to 17 across ~180 builds — but the acceptance is `miss burst=0` and 17 is not 0. Next flash attributes each miss to its step (`O2m:` line) and carries the S3L_STRESS robot keybed. Open: stepped build costs 2.2-3.5 M total vs the monolith's 2.1 M; the demo loop rebuilds on every gate flip (~2 builds/s background); the once-per-second ~8.5 k µs gap (report-print suspect, playbook 12 shape). |
