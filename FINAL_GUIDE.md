...
| **O3** | C9 | per-parameter incremental refresh (derived field map) | **DONE 2026-08-19 — PROVEN ON SILICON (data/b15_o3_silicon.md).** `PARAM: edits=10146 builds=665 unknown=0 pubretry=0 applymax=291393` over a 425 s robot run with a KNOBSTORM phase. **unknown=0** (every id in the class table), **pubretry=0** (no build ever stranded), **15.3x coalescing** — 10,146 edits cost 665 rebuilds, which is C9's "as many parameters as you please, at the same time" met by construction. The warm recall's worst case (291,393 cyc) FITS the measured slack (420k-1.3M) and `forced=0` throughout, so the budget never had to force a step. Host: median parameter moves 32 of 12,276 coef bytes; 2,036/2,040 warm==cold, the four that differ are latches that settle; `eb_param_class.h` GENERATED and re-derived every run; 57 teeth across 5 gates (`sh tools/engineb/o3_gates.sh`). ⚠ COST, MEASURED: a key occasionally waits 9 blocks instead of 2 (78 of ~2,000 presses) when it queues behind a knob build — a PRIORITY POLICY the user may overturn. |
| **O4** | B3/B4 | worst-case headroom | **OPEN, BUT NARROWED TO FOUR PATCHES AND ONE MODULE (data/b16_o4_delay_t5.md).** The deficit is 259 cyc/sample (b15 §2: 73,443 blocks produce 426.3 s of audio in 446.4 s real, `drift` is MILLISECONDS). It is NOT spread: `v1` is flat within ~80 cyc and `wait=5`, so all ~1,900 cyc/sample of variance is in `fx`, core 1's master pass. Pairing `FXP:` with `pat=`, the hot patches are **5, 16, 21, 49 — exactly the four DELAY TYPE 5 patches in the bank**, which is also b6's independently named over-budget set. `eb_delay_t5.c` is 553 lines with 12 ring accesses against 245/269 lines and 6 for t1/t23 — **2x the taps and arithmetic**, which also supports b6's WITHDRAWAL of the PSRAM attribution (moving-tap 29.8 cyc/tap; rings moved to internal SRAM made it 94 cyc WORSE). **60 of 64 patches already comply**, so splitting the master chain across cores would cost every patch 5.8 ms of latency to fix four — optimising one module costs none. ⚠ CIRCUMSTANTIAL until measured: a five-stage profiler is in `eb_master.c` (`EB_MSPROF`, default 0, trunk gate GREEN and assembly byte-identical when off). **PREDICTION STATED BEFORE THE RUN: stage 1 (DELAY dispatch), on those four patches, ~1,500 cyc/sample above the other sixty.** If it lands in reverb or spreads, this is wrong and split-across-cores returns. Keep split 7. **STEADY-STATE HALF — SILICON RESULTS IN (b27/b30/b31):** (1) b16 CONFIRMED on the board — the excess IS in the DELAY stage on patches 5/16/21/49 (+835..+1480 cyc), not reverb, so the master split stays dead and the t5 arithmetic is the worst-case lever. (2) VCA move re-measured on silicon: `vca/vcf`=0.542, NOT QEMU's 0.352, so VCA-only OVERSHOOTS — the cut must be a SUBSET of voices (b27). (3) THREE bit-exact voice-side levers LANDED and proven (L-B LFO tail, L-A set_pitch, dead stores; forkbit EXACTLY 0; make engineb GREEN) — ~118 µs saved at idle on silicon (b30/b31), which NARROWS the +197 µs deficit to ~79 µs but does not close it. So the subset VCA move is still needed, now smaller. NEXT: read the clean-build `B4dur quiet` (no-profiler flash staged), then build+host-prove the subset VCA move (forkbit) before flashing. |
