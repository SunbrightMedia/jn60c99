...
| **O4** | B3/B4 | worst-case headroom CLOSED — the only holder of the deadline deficit | **OPEN. TARGET CONFIRMED ON SILICON (data/b15_o3_silicon.md §2).** 425 s run: **73,443 blocks produce 426.3 s of audio in 446.4 s of real time** — the engine is **4.8 % behind** and the drift counter shows **20.1 s lost in one run** (`drift` is MILLISECONDS: `+20114` = 20.1 s). `B4dur` agrees with real elapsed to **0.03 %**, so the meter is sound. Restated: mean block **6,081 µs vs a 5,805 µs period = 276 µs over = 259 cyc/sample**, actual ~5,701 against the 5,442 budget. ⚠ b15 §2 first RETRACTED this target and was itself wrong twice — `drift` misread as µs, and `t=` (audio time, derived from the BLOCK COUNT) used as wall clock, which is circular. See playbook 68. Next, in order: (a) prologue (`S3L_TIME_PROLOGUE`, still 0); (b) the delay arm — this run shows `FXP: fx=2,4xx-4,3xx` swinging ~1,900 cyc/sample across patches while `v1` is flat and `wait=5`, so core 1 is still the constraint; (c) the 4-vs-18 classification mismatch; (d) then the lever — master-chain split across cores vs arm hunt. Evidence base b6_split_sweep.md; keep split 7. |
