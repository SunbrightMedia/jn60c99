...
| **O4** | B3/B4 | worst-case headroom | **OPEN — AND ITS TARGET IS WITHDRAWN (data/b15_o3_silicon.md §2).** b14 stated 306 µs/block and 287 cyc/sample over budget. Both are RETRACTED: they came from `B4dur`, and b14's "corroboration" of it misread the drift field's MICROSECONDS as seconds, so two numbers that disagree by **1,000x** were reported as agreeing to 0.4 %. What the board actually says: **73,443 blocks in 425 s = 5,787 µs per block, against a 5,805 µs period.** The engine KEEPS UP; drift accrues at 0.27 µs/block (0.005 %), not 276. Block count, elapsed time and the drift counter all agree; only `B4dur` dissents, by ~3.5 % across all three classes — so the MEAN is the bug. **FIRST STEP IS THEREFORE TO FIX OR RETIRE `B4dur`**, not to hunt cycles: every lever would be chosen against a number 1,000x wrong. Still real and unexplained: `gap=9,000-12,000 µs` worst block-to-block (tagged to printf), and rare `miss` events now unattributed again. Evidence base b6_split_sweep.md; keep split 7. |
