...
| **O2** | C10 | chunked patch change — THE fix for B4's counted misses | **BUILT AND GATED 2026-08-18, UNFLASHED.** Fourteen bounded steps, one per block (~81 ms per program change), one atomic publish. Step budget is measured, not chosen: worst step ~440k cycles against core 0's ~650k measured idle (b6). tools/engineb/chunk_gate.py holds the chunked build byte-identical to the monolith over all 64 patches, 5 teeth caught; make engineb GREEN after the eb_coefs split. Notes wait on a live build and, being queued by O1, wait rather than vanish; a mid-build request restarts with the newer patch and is counted. **NOT DONE until the board shows `B4: miss` not incrementing across a program change on all 64 patches with CRC still matching.** |
