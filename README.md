...
| Polyphony (8 voices) + note-on driver | 98% | All 8 voices render; all 622 per-voice offsets verified. **Note path now transcribed from the binary (2026-07):** the voice-trigger writes the key CV to M.CV (304) via the tid-32 pitch LUT with step = MIDI note — LUT32 is the plugin's semitone table with per-key analog detune; 4448 stays at its static −4.75 (engine_init) so `Hz = 440·2^((note−69)/12 + ε)`. Velocity → 6864..6912 = v/127. **Validated against the plugin's own runtime dump** (held note 92 vel 107): M.CV, smoother, velocity all bit-exact; the oscillating pitch-mod sum lands inside the dump's own snapshot band. This fixed three driver bugs (pitch reference; unwritten velocity; bypassed M.CV that disabled VCF key follow + portamento). 64-preset smoke: 63/64 clean. |
...
| Preset / bank decode | 100% | Deserializer-proven and **audited handler-by-handler against the decompile** (the CDSPJu60*Voice section handlers, not the registry vtable, dispatch preset values). Capture-free load of record 0 now reproduces the verification oracle **58/58 bit-exact** (beating the live capture at 52/58). The audit fixed 5 real decoder bugs: VCA TONE tid21→24 (bipolar LUT24; fixed record-40's hot render), DCO NOISE tid12→54 (LUT54, no gate — 14 presets had inflated noise), LFO RATE 3-slot fan-out (+2064 LFO Noise Mix, +1072 tempo-rate = LUT53[bpm·10]·LUT48[step]; 34/64 presets use tempo-sync), HPF 4-slot write (+10256 engage switch; HPF now engages), LFO Delay Sw. 64/64 factory patches render clean. |
...
| Host layer — preset loader + RT synth API + VST3 | 85% | Preset loader + CLI host (`host/juno_render.c`) + **real-time polyphonic synth API** (`host/juno_synth.c`: create-at-host-SR / load / note-on-off / set-param / process-block, 8-voice allocation). **VST3 wrapper written** (`vst3/`: processor + controller + factory + CMake) against the Steinberg SDK; the engine-facing contract is validated (create→load→MIDI→process at 44.1 kHz with the capture file absent). Remaining: compile/link in an SDK-present environment + ship the factory bank in the bundle Resources. |
| **Capture-seed elimination** | ~92% | The **product is now capture-free**: `host/juno_render.c` + `host/juno_synth.c` (and the VST3 build) seed engine state from `juno_capture_free_seed()` — `sub_3A66B0` construction defaults + `sub_388170` registration defaults (the one function Hex-Rays couldn't lift; `.rdata`-sourced) + the Mod/Vel/Bend-Sens + M.CV framework constants (LUT outputs = `.rdata` literals) + the 98 `.rdata` FX-filter templates + 48 HALL2 reverb coeffs. The whole binary links and renders with `runtime_coeffs_data.c` excluded. Verified vs the record-0 oracle: the capture-free seed reproduces **56/58** voice-0 oracle offsets — **better than the capture seed (50/58)**, which carried another preset's residue. The 2 remainders are documented: `1072` LFO-tempo-rate (note-on/tempo state) and `2064` LFO Noise Mix (one preset param whose bank decode position has a value-collision). The capture file is retained only as a regression oracle (`-DJUNO_USE_CAPTURE`). |
| **Sound accuracy** | — | **Corrected (2026-07):** an earlier version of this row claimed "no remaining bug" and that the velocity handler was an inert dead end. Rendering a capture-free SQ-ARPG C-major chord proved otherwise — it came out as sub-audio rumble, which isolated **two real note-on/driver bugs** (DCO pitch reference; unwritten VCA velocity gate — see the Polyphony row). Both are fixed and pitch/level are now verified by measurement (equal-temperament to a few cents; 59/64 patches render clean). The static DSP *coefficients* remain bit-exact-verified; these were driver-layer bugs the block-level checks didn't cover. Full sonic fidelity to the plugin is still **unconfirmed** — the fixes were validated against music theory (C4 = 261.6 Hz), not against a reference render of any specific patch, and no per-patch capture exists to prove bit-exactness. |
