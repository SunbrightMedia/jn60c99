...
| **Capture-seed elimination (in progress)** | 52% | The render seeds engine state via `juno_runtime_coeffs_apply` — originally **279 coefficients memory-captured from a *different* preset ("PD The Juno Pad")**. **146 are now sourced capture-free from the binary's `.rdata`** (98 shared FX filter templates → `src/juno_fx_filter_coeffs.c`; 48 HALL2 reverb tank/allpass → `src/juno_reverb_coeffs.c`; provenance verified in `data_sections/seg_rdata_935650.bin`, render bit-identical). **133 voice-region coefficients remain** in the capture — these are note-on runtime state + global analog-cal constants, not preset params. A full sensitivity analysis (`docs/SENSITIVITY_ANALYSIS.md`) shows they barely affect the sound: 78 are sonically irrelevant, the rest are loader-overwritten, level-only (normalized away), self-converging state, or confirmed-correct global constants (master tune, duty, switches). |
