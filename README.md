...
| Panel→engine param coverage | 60% | **24 of ~61 panel-exposed params bound** to engine offsets (each capture-validated). Added the missing pulse-osc level (DB770→4208) + VCA tone (DB793→9584). Remaining ~37 sit at init defaults: the FX sends (DB794-797) are **host-side** (red-black tree, not statically bindable — same class as the Chorus CV); the rest are switches/selectors + BEND. |
