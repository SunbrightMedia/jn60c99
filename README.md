...
| Polyphony (8 voices) + note-on driver | 95% | All 8 voices render; all 622 per-voice offsets verified. **Two real note-on/driver bugs fixed (2026-07):** (1) DCO pitch used a wrong 22380.1 Hz reference (−5.669 oct); the pitch table is A440-referenced, so notes were an octave sharp with the capture and collapsed to sub-audio rumble without it — fixed to `Hz = 440·2^((note-69)/12)`, verified within ~3 cents across the keyboard on a bright patch. (2) `note_on` never wrote the per-voice velocity slots (6864/6880/6896/6912) that gate the VCA, so notes were 20–40× too quiet — now `juno_note_on_vel` writes v/127 and the synth passes real velocity. Broad smoke test: 59/64 factory patches render clean (0 silent, 0 NaN), the rest are LFO-heavy pads/one hot patch (unverified vs a reference). |
