...
| Panel→engine param coverage | 100% | **Every panel parameter is bound and capture-validated bit-exact.** Complete DB→engine map (`refs/db_engine_map_full.json`) from the static positional descriptor walk (`sub_33BFC0`; DB=750+position). Applied: 24-param LUT map + pulse/VCA-tone + LFO KEY TRIG + PWM-source demux + portamento (tid7) + tempo-sync + BEND RANGE (`LUT21(step+160)`) + arp + 5 FX selectors + FX-A DELAY + **REVERB LEVEL** (10759440, curve-22) + **VCF LFO MOD** (7344, tid47) + **VCA LEVEL** (101072 "Patch Level", tid49 dB curve) + **EFFECT DEPTH** (102576, step/255). The last 4 "host-side"-flagged params were all found statically and round-trip bit-exact against the full SY Poly capture — *nothing* required a runtime capture. |
