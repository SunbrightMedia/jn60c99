...
| Panel→engine param coverage | 95% | **Complete DB→engine map** (`refs/db_engine_map_full.json`, 128 indices, 41 capture-validated bit-exact) from the static positional descriptor walk (`sub_33BFC0`; DB=750+position). Applied: 24-param LUT map + pulse/VCA-tone + LFO KEY TRIG + PWM-source demux + portamento (tid7) + tempo-sync + **BEND RANGE** (`LUT21(step+160)`) + arp + 5 FX selectors + **FX-A DELAY** + **REVERB LEVEL** (`10759440`=curve-22 — was wrongly called host-side; binding it brought L/R correlation to 0.502 vs reference 0.491). The last 3 are **EditController/host-layer** (`RangeParameter` normalization, a different subsystem than the DSP engine): VCA LEVEL (normalized out of renders), EFFECT DEPTH (only used by chorus-mode FX-A; delay presets use the delay On/Off), and VCF LFO MOD (its DB754 label is unreliable; needs a non-default capture to bind). Every DSP-engine param is bound. |
