...
| Panel→engine param coverage | 88% | **Complete DB→engine map** (`refs/db_engine_map_full.json`, 128 indices, 41 capture-validated bit-exact) from transcribing the static positional descriptor walk (`sub_33BFC0`; DB=750+position; offset+tableId in `juno_param_table.h`). Every statically-bindable panel param is now applied: the 24-param LUT map + pulse/VCA-tone + LFO KEY TRIG + PWM-source demux + portamento (tid7) + tempo-sync + **BEND RANGE** (transform `LUT21(step+160)` resolved) + arp + the 5 FX selectors + **FX-A DELAY activation**. Remaining genuinely **host-side / not statically bindable**: VCA LEVEL, FX wet sends (DB794-797), DB754 VCF LFO MOD — these ride the (correct-for-tested-patches) defaults. |
