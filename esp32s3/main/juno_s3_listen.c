 ...
 * layout. That blind check let a real fault through: MEASURED,
 * sizeof(eb_render_state) is 735,332 under the firmware's default flags and
 * 736,744 under -DEB_DCO_WT=1, and offsetof(wt) is 3,016 -- INSIDE the
 * 6,808-byte voice prefix load_coefs() memcpy's in. The wavetable state
 * received trunk bytes, rpos became a wild index, and the board died with an
 * MMU entry fault on its first sounding sample.
 *
 * THE CHECK IS NOT AN EQUALITY, and the first version of it was, which is why
 * it failed a build that was in fact sound. The blob is generated on the HOST,
 * where a pointer is 8 bytes; here it is 4. eb_render_state legitimately
 * differs by that pointer slack -- 96 bytes today. What must NOT differ is the
 * STRUCTURE: EB_DCO_WT inserts 1,412 bytes of new members in the middle, and
 * that is two orders of magnitude above the slack.
 *
 * C99 has no _Static_assert, so this is the negative-array form. */
#define S3L_PTR_SLACK 256
typedef char s3l_rstate_layout_check
    [(sizeof(eb_render_state) >= S3L_RSTATE_SZ &&
      sizeof(eb_render_state) - S3L_RSTATE_SZ < S3L_PTR_SLACK) ? 1 : -1];
typedef char s3l_voice_prefix_check
    [(S3L_VOICE_SZ <= S3L_RSTATE_SZ) ? 1 : -1];

/* EB_DCO_WT is refused OUTRIGHT rather than bounded, because the slack test
 * above would happen to catch today's 1,412 bytes but says nothing about a
 * future member that is small AND misplaced. The blob must be regenerated by
 * a generator built with the same flags before this build can be correct. */
#if EB_DCO_WT
#error "s3_listen.bin is a trunk-layout snapshot; EB_DCO_WT moves eb_render_state. Regenerate the blob under the same flags first (tools/engineb/gen_listen_coefs.py)."
#endif
